...
```
 ...
 3. Extension HMAC Authentication
```bash
# Extensions authenticate dynamically via X-Extension-* headers signed with per-device HMAC-SHA256:
curl -X POST http://localhost:8000/extension/analyze \
     -H "Content-Type: application/json" \
     -H "X-Extension-Timestamp: <UNIX_TIMESTAMP>" \
     -H "X-Extension-Signature: <HMAC_SHA256_HEX>" \
