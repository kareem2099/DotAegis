...
class MetricsCollector:
    ...
    def get_metrics_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'db_connections_active': self.db_connections_active._value.get() if hasattr(self.db_connections_active, '_value') else 0,
            'model_training_samples': self.model_training_samples._value.get() if hasattr(self.model_training_samples, '_value') else 0,
            'service_health': self.service_health_status._value.get() if hasattr(self.service_health_status, '_value') else 1,
            'uptime_seconds': self.service_uptime._value.get() if hasattr(self.service_uptime, '_value') else 0
        }

    def _calculate_cache_hit_rate(self) -> float:
        """Calculate cache hit rate."""
        hits = self.cache_hits_total._value.get()
        misses = self.cache_misses_total._value.get()
        total = hits + misses
        if total == 0:
            return 0.0
        return hits / total
